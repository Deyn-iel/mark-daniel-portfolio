...
const icons = {
  sparkle:
    '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="m12 3 2.4 6.6L21 12l-6.6 2.4L12 21l-2.4-6.6L3 12l6.6-2.4L12 3Z" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/></svg>',
  ...
  close:
    '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>',
...
function addMessage(text, from, prompt) {
  ...
  if (prompt?.href) {
    ...
    if (prompt.href.startsWith("https://")) {
      link.target = "_blank";
...
async function submitMessage(text) {
  ...
  try {
    ...
    const prompt = prompts.find(
      (prompt) => prompt.text.toLowerCase() === text.toLowerCase(),
    );
    const message = addMessage(
      data.reply,
      "assistant",
      data.href ? data : prompt,
    );
    message.querySelector(".chat-message-name").textContent =
      data.mode === "ai" ? "On-device AI" : "Portfolio guide";
    history = [
      ...conversation,
      { role: "assistant", content: data.reply.slice(0, 1000) },
    ].slice(-4);
  } catch {
    pending.remove();
    addMessage(
      "Please ask about Mark’s projects, skills, education, or contact details.",
      "assistant",
      { href: "#contact", link: "Contact Mark" },
    );
  } finally {
    busy = false;
...
widget
  ...
  .forEach((button) =>
    button.addEventListener("click", () =>
      submitMessage(prompts[Number(button.dataset.prompt)].text),
    ),
  );
